...
\subsection{The tableau calculus $\secondo$}\label{tableau_ph2}
...
Both the employer and the worker of our system rely on Prolog for their computing cores: \verb$alct1.pl$ and \verb$alct2.pl$. The former implements the first phase ($\primo$), while the latter implements the second phase of the calculus ($\secondo$).
...
The \emph{employer} handles the query in \verb$Emplojer.java$, a piece of Java code which presents it to \verb$alct1.pl$, the Prolog core implementing $\primo$ (case 1). Every time that an open branch is generated, \verb$alct1.pl$ calls (case 2) \verb$Phase1RMIStub.java$, another Java code which will send it to the correct \emph{worker}.
...
%FIXME
As stated in \ref{ssec_jasper}, since \emph{DysToPic} relies on a Java interface to Prolog, it was natural to design also the distribution feature around Java APIs\footnote{With Java versions before Java 5.0 developers had to compile RMI stubs in a separate compilation step using \texttt{rmic}. Version 5.0 of Java and beyond no longer require this step.}.\\

The RMI architecture makes use of a \emph{Registry} (\verb$java.rmi.registry.Registry$) that provides methods for storing and retrieving remote object references bound with arbitrary string names.\\
...
Due to a limitation in the RMI protocol, we had to make the repository expose a method called \texttt{proxyRebind(String name, Remote stub)}, which binds the Remote object that is supplied to it to the local RMI registry.
...
\section{An example of execution}

The RMI registry is launched on the machine 'A' on port 'x'.
The repository (AnswersRepository.java) is then executed on the same machine. It binds itself on the RMI registry and goes idle.

Then an arbitrary number of workers (Phase2Worker.java) is launched on an arbitrary number of machines (let's say machines 'B' to 'N') - for instance one worker per CPU\footnote{We chose to use the convention that each worker has a different name, which has to be a string starting with "Worker" (i.e. "Worker1", "Worker2", and so on).}.
Each worker binds itself (through the proxyRebind() method of AnswersRepository.java) to the RMI registry and then starts its single instance of SolverThread.
SolverThread.java creates a SICStus object with the code of alct2.pl (which corresponds to $\secondo$) and then waits until a request (Request.java) comes from the employer.

The employer (Emloyer.java) is then started on the machine 'A', and receives from the user a queryString (which contains both the $KB$ and the actual query to be verified). It retrieves a list of each worker bound to the RMI registry and writes it onto a file workersList.txt (it will be necessary for alct1.pl). Then it spawns a new thread (Phase1Thread.java) which creates a SICStus object with the code of alct1.pl (implementing $\primo$) and feeds it the queryString (via Jasper).
The control passes to the Prolog code, which manipulates the queryString and generates various branches ($\primo$). Some may be closed, others may be open. If every branch is closed, the computation ends affirmatively: the query does entail from the $KB$. Otherwise, alct1.pl invokes (via Jasper) the method solveViaRMI(String workerName, String query) of Phase1RMIStub.java. The query corresponds to the branch to be verified, while the workerName is extracted from workersList.txt\footnote{This is done FIFO, reading a name from the top of the file, and writing it on the bottom, and allows a simple form of load balancing.}.
Phase1RMIStub.java's main thread, then spawns a new thread (PrologWorker.java) and increases the counter of the open branches (Counter.java).
PrologWorker.java is the employer's side stub of a worker, which the employer can use to solve the query, i.e. to ask the worker whether the branch is open or closed.
Meanwhile, the employer's main thread (just after spawning Phase1Thread.java) enters a loop in which polls the repository until a global answer can be found. This happens when:
\begin{itemize}
\item one worker has answered 'false', invalidating the whole entailment (the method tell() returns false) - here the Employer answers 'NO';
\item all the open branches (counted by Counter.java) have been tested and every worker has answered 'true' (the method tell() still returns false) - here the Employer answers 'YES'.
\end{itemize}
Until then, the employer just remains in a busy wait condition.

When the PrologWorker thread of the employer calls solve(String remoteQuery) on a worker, for instance "Worker2", the latter just asks its solver (Solver.java) to enqueue the request.
The SolverThread, then, will then take the request from the queue and elaborate it through alct2.pl.
This elaboration yields an answer of the form 'true' or 'false', which is then sent (through the method store(Boolean answer)) to the repository. The SolverThread then restarts its infinite loop and looks for another request that may or may not have come from the employer in the meantime.
