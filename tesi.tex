...
\subsection{The previous theorem prover: PreDeLo}\label{predelo}
\emph{PreDeLo} \cite{conf/aiia/GiordanoGJOP13} is a theorem prover for Preferential Description Logics, nonmonotonic extensions of standard DLs based on a typicality operator $\tip$, which enjoys a preferential semantics.

\emph{PreDeLo} is a Prolog implementation of labelled tableaux calculi for such extensions, and it is able to deal with $\alctmin$ (the preferential extension of the basic DL $\alc$), as well as with the preferential extension of the lightweight DL $DL-Lite_{core}$.
...
In \cite{conf/aiia/GiordanoGJOP13}, a tableaux calculus for checking entailment in the rational extension of $\alc$ is also introduced.
...
The pre--existing theorem prover is %%TODO: completare
