...
The logic is an extension of the Description Logic ALC, adding a typicality operator $\tip$, which allows to define concepts such as $\textbf{T}(C)$, i.e. selecting the "most typical" or "most normal" instances of the concept $C$. This allows the knowledge bases to contain subsumption relations (e.g. "$\textbf{T}(C) \sqsubseteq D$" - the typical members of $C$ are instances of the concept $D$). Moreover, the use of a "minimal model" semantics (i.e. models that maximise typical instances of concepts), allows new forms of non-monotonic reasoning on prototypical properties and defeasible inheritance.
Expanding a pre--existing sequential software, the application is based on a SICStus Prolog implementation of the tableaux calculi $\nuovoc$, wrapped by a Java interface which relies on the Java RMI APIs for the distribution.
