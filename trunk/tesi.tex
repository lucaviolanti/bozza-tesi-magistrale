...
Our work aims to improve \emph{PreDeLo} by tackling this limitation: to do so we developed a new theorem prover named \emph{DysToPic} (illustrated in chapter \ref{chap_dystopic}), which is a distributed software, capable of exploiting multiple computing units in parallel to performing its inferences.
...
\begin{figure}[htp]
\begin{center}\includegraphics[scale=0.25]{img/union_rule.png}\end{center} %%FIXME: ripristinare l'albero!
%	\begin{prooftree}
%		\rootAtTop
...
\begin{figure}[htp]
\begin{center}\includegraphics[scale=0.32]{img/box_rule.png}\end{center} %%FIXME: ripristinare l'albero!
%\footnotesize
%\begin{prooftree}
%	\rootAtTop
...
Each worker binds itself (through the \texttt{proxyRebind} method of \texttt{AnswersRepository.java}) to the RMI registry and then starts its single instance of \texttt{SolverThread}.
...
\item Afterwards, the \emph{employer} (\texttt{Emloyer.java}) is started on the machine `A', and receives from the user a \texttt{queryString} (which contains both the $KB$ and the actual query to be verified). It retrieves a list of each worker bound to the RMI registry and writes it onto a file (\texttt{workersList.txt} - this file is the means for communicating the names to \texttt{alct1.pl}).

\item[5a.] Then the \emph{employer} spawns a new thread, \texttt{Phase1Thread.java}.

This creates a SICStus object and loads it with the code of \texttt{alct1.pl}, which contains the rules of $\primo$. Then the \texttt{queryString} is passed to this SICStus object as a parameter (via Jasper). The query is executed and the control passes to the Prolog code (\texttt{alct1.pl}), which starts manipulating the \texttt{queryString} and generating various branches. Some may be closed, others may be open:
	...
	\item Otherwise, for each \emph{open} branch found, \texttt{alct1.pl} invokes (via Jasper) the method \texttt{solveViaRMI(String workerName, String query)} of \texttt{Phase1RMIStub.java}. The \texttt{query} corresponds to the branch to be verified, while the \texttt{workerName} is extracted from the file \texttt{workersList.txt}\footnote{This is done FIFO, reading a name from the top of the file, and writing it on the bottom, and allows a simple form of load balancing.}.
	\end{enumerate}
\texttt{Phase1RMIStub.java}'s main thread, then spawns a new thread (\texttt{PrologWorker.java}) and increases the counter of the open branches (an object of class \texttt{Counter.java}).
...
\item[5b.] Meanwhile, the \emph{employer}'s main thread (just after spawning \texttt{Phase1Thread.java}) enters a loop in which it polls the \emph{repository} until a global answer can be found. This happens when:
\begin{itemize}
...
In this thesis we presented a distributed theorem prover implementation for $\nuovoc$.\\
...
Since the previously developed theorem prover, \emph{Predelo}, showed promising results in the demonstration of complex formulas for this calculus, we decided to improve such approach.

\emph{DysToPic} is a Java software, built around two Prolog cores which implement the two phases of $\nuovoc$ ($\primo$ and $\secondo$). It relies on the RMI technology for the distribution of the calculus and it is multi--threaded for its parallelisation.\\
