...
Each \emph{worker} then asks the RMI registry to lookup for the \emph{repository} (steps \circled{1} and \circled{2}) and uses \texttt{proxyRebind} to bind itself (steps \circled{3} and \circled{4}) to the RMI registry as \texttt{"Worker*"}, where \texttt{*} is any string, i.e. both \texttt{"Worker1"} and \texttt{"WorkerFoo"} are valid names.
